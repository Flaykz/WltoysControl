...
void nRF24::rxMode(uint8_t initialFreq)
{
  setCeLow();
...
void nRF24::txMode(uint8_t initialFreq)
{
  setCeLow();
  writeRegister(CONFIG, _BV(EN_CRC) | _BV(CRCO));	// Enable CRC (2bytes)
  writeRegister(EN_AA, 0x00);		// Disable auto acknowledgment
  ...
  Serial.write("Try to switch banks "); Serial.print(readRegister(STATUS)); Serial.write("\n");
  if (readRegister(STATUS) & 0x80) {
